...
# আপনি চাইলে এই লিস্টে নতুন ক্যাটাগরির নাম যোগ করতে পারেন
CATEGORIES = [
    "Monthly Report", 
    "Annual Report", 
    "Project Report", 
...
if choice == "নতুন ফাইল আপলোড (Upload File)":
    ...
    if uploaded_file is not None:
        # ক্যাটাগরি -> বছর -> মাস অনুযায়ী ফোল্ডার তৈরি হবে
...
# পেজ সেটআপ
st.set_page_config(page_title="মাসিক রিপোর্ট পোর্টাল", page_icon="📊", layout="wide")

# ফাইল সংরক্ষণের ফোল্ডার
UPLOAD_DIR = "reports_data"
if not os.path.exists(UPLOAD_DIR):
    os.makedirs(UPLOAD_DIR)

st.title("📊 মাসিক রিপোর্ট পোর্টাল (Monthly Reports)")
st.markdown("---")

# সাইডবার মেনু
menu = ["রিপোর্ট দেখুন (View Reports)", "নতুন রিপোর্ট আপলোড (Upload Report)"]
choice = st.sidebar.selectbox("মেনু নির্বাচন করুন", menu)

# --- নতুন ফাইল আপলোড সেকশন ---
if choice == "নতুন রিপোর্ট আপলোড (Upload Report)":
    st.subheader("📤 নতুন রিপোর্ট জমা দিন")
    ...
    if uploaded_file is not None:
        save_path = os.path.join(UPLOAD_DIR, year, month)
        os.makedirs(save_path, exist_ok=True)
        ...
        st.success(f"✅ ফাইল জমা হয়েছে: `{uploaded_file.name}` ({year} - {month})")

# --- ফাইল দেখার ও প্রিভিউ সেকশন ---
elif choice == "রিপোর্ট দেখুন (View Reports)":
    st.subheader("📂 সংরক্ষিত রিপোর্টসমূহ")
    
    col1, col2 = st.columns(2)
    with col1:
        years_available = sorted(os.listdir(UPLOAD_DIR)) if os.path.exists(UPLOAD_DIR) else []
        selected_year = st.selectbox("বছর সিলেক্ট করুন", years_available if years_available else ["কোনো ডেটা নেই"])
    
    with col2:
        year_path = os.path.join(UPLOAD_DIR, selected_year) if selected_year != "কোনো ডেটা নেই" else ""
        months_available = sorted(os.listdir(year_path)) if os.path.exists(year_path) else []
        selected_month = st.selectbox("মাস সিলেক্ট করুন", months_available if months_available else ["কোনো ডেটা নেই"])
        
    if selected_year != "কোনো ডেটা নেই" and selected_month != "কোনো ডেটা নেই":
        target_dir = os.path.join(UPLOAD_DIR, selected_year, selected_month)
        files = os.listdir(target_dir) if os.path.exists(target_dir) else []
        
        if files:
            selected_file = st.selectbox("রিপোর্ট সিলেক্ট করুন", files)
            file_full_path = os.path.join(target_dir, selected_file)
            ...
            st.info("এই মাসের কোনো ফাইল পাওয়া যায়নি।")
